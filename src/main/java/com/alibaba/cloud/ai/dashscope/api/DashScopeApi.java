...
public class DashScopeApi {
    ...
    public ResponseEntity<DashScopeApiSpec.ChatCompletion> chatCompletionEntity(DashScopeApiSpec.ChatCompletionRequest chatRequest,
        ...
        System.out.println();
        log.info("DashScopeApi.chatCompletionEntity, 调用模型入参:  " + System.lineSeparator() + " {}", JackJsonUtil.toJsonStr(chatRequest));
        System.out.println();
        ...
        // @formatter:off
        ResponseEntity<DashScopeApiSpec.ChatCompletion> entity = this.restClient.post()
        ...
        System.out.println();
        log.info("DashScopeApi.chatCompletionEntity, 调用模型出参:  "+System.lineSeparator()+" {}", JackJsonUtil.toJsonStr(entity));
        System.out.println();
        return entity;
        // @formatter:on
    }
    ...
    public Flux<DashScopeApiSpec.ChatCompletionChunk> chatCompletionStream(DashScopeApiSpec.ChatCompletionRequest chatRequest,
        ...
        System.out.println();
        log.info("DashScopeApi.chatCompletionStream, 调用模型入参:  " + System.lineSeparator() + " {}", JackJsonUtil.toJsonStr(chatRequest));
        System.out.println();

        // 标记当前是否正在处理工具调用（function call）的流式输出
        ...
        return this.webClient.post().uri(chatCompletionUri).headers(headers -> {
                ...
                .retrieve()
                // 将响应体按行解析为字符串流（SSE 每行一个事件）
                .bodyToFlux(String.class)
                // 遇到结束标记（如 "data: [DONE]"）时停止接收
                .takeUntil(SSE_DONE_PREDICATE)
                // 过滤掉结束标记行，只保留有效数据
                .filter(SSE_DONE_PREDICATE.negate())
                // 将每行 JSON 字符串解析为 ChatCompletionChunk 对象
                .map(content -> {
                    // 先检查是否为错误响应
                ...
                // 按工具调用边界分组：工具调用期间的 chunk 归为一组，普通响应每个 chunk 单独一组
                .windowUntil(chunk -> {
                    // 如果正在处理工具调用，且检测到工具调用结束，则关闭窗口
                    if (isInsideTool.get() && chunkMerger.isStreamingToolFunctionCallFinish(chunk)) {
                        isInsideTool.set(false);
                        return true;
                    }
                    // 如果不在工具调用中，每个 chunk 都关闭窗口（即每个 chunk 单独一组）
                    return !isInsideTool.get();
                })
                // 将每个窗口内的 chunk 合并为一个完整的 chunk
                .concatMapIterable(window -> {
                    // 使用 chunkMerger.merge 将窗口内的所有 chunk 合并
                ...
                // 展平 Mono 流，得到最终的 Flux<ChatCompletionChunk>
                .flatMap(mono -> {
