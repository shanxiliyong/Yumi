...
public class NodeExecutor extends BaseGraphExecutor {
     ...
     * 执行流程：
     * 1. 设置当前节点 ID
     * 2. 获取节点的异步动作（AsyncNodeActionWithConfig）
     * 3. 检查节点是否存在
     * 4. 处理可中断动作（InterruptableAction）
     * 5. 触发 NODE_BEFORE 事件监听器
     * 6. 执行节点动作（异步）
     * 7. 处理执行结果或错误
     * 
     * @param context 图运行上下文
     * @param resultValue 原子引用，用于存储最终结果
     * @return Flux<GraphResponse<NodeOutput>> 节点执行结果流
     */
    private Flux<GraphResponse<NodeOutput>> executeNode(GraphRunnerContext context,
        ...
        try {
            ...
            // 4. 处理可中断动作（如 Human-in-the-loop 审批）
     ...
     * 
     * 处理流程：
     * 1. 检查是否嵌入了 Flux 流（流式响应）
     * 2. 检查是否嵌入了 ParallelGraphFlux（并行图）
     * 3. 检查是否嵌入了 GraphFlux（子图）
     * 4. 处理可中断动作的 interruptAfter 钩子
     * 5. 合并节点更新的状态
     * 6. 检查是否需要在边执行前中断
     * 7. 计算下一个节点 ID
     * 8. 构建节点输出和检查点
     * 9. 触发 NODE_AFTER 事件监听器
     * 10. 递归调用 mainGraphExecutor 继续执行
     * 
     * @param context 图运行上下文
     * @param updateState 节点更新的状态
     * @param resultValue 原子引用，用于存储最终结果
     * @return Flux<GraphResponse<NodeOutput>> 处理结果流
     */
    private Flux<GraphResponse<NodeOutput>> handleActionResult(GraphRunnerContext context,
        ...
        try {
            ...
            // 1. 检查是否嵌入了 Flux 流（如 ReactAgent 的流式输出）
            ...
            // 2. 检查是否嵌入了 ParallelGraphFlux（并行节点返回）
            ...
            // 4. 处理 interruptAfter 钩子（在 apply() 之后，状态合并之前）
            String currentNodeId = context.getCurrentNodeId();
            AsyncNodeActionWithConfig action = context.getNodeAction(currentNodeId);
            if (action instanceof InterruptableAction) {
                Optional<InterruptionMetadata> interruptMetadata = ((InterruptableAction) action)
            ...
            // 6. 检查是否需要在边执行前中断（interruptBeforeEdge 配置）
            ...
            // 10. 递归调用 mainGraphExecutor 继续执行下一个节点
