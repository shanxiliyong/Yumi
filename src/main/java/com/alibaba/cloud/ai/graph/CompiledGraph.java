...
public class CompiledGraph {
    ...
    /**
     * 将 GraphResponse 流扁平化为 NodeOutput 流，并保持原始顺序
     * 
     * 处理逻辑：
     * 1. 如果响应已完成（done）：提取 NodeOutput 结果
     * 2. 如果响应出错（error）：转换为错误信号
     * 3. 否则：将 CompletableFuture 转换为 Flux 流
     * 
     * 使用 flatMapSequential 确保即使异步操作完成顺序不同，
     * 最终输出的顺序仍与原始响应顺序一致
     * 
     * @param responses GraphResponse 响应流
     * @return NodeOutput 输出流（保持顺序）
     */
     ...
     * 
     * 执行流程：
     * 1. stream(inputs, config) - 启动流式执行，返回 Flux<NodeOutput>
     * 2. .last() - 取最后一个输出（最终状态）
     * 3. .map(NodeOutput::state) - 提取 OverAllState
     * 4. .block() - 阻塞等待结果
     * 5. Optional.ofNullable() - 包装可能为 null 的结果
     * 
     * @param inputs 输入参数
     * @param config 运行配置
     * @return 包含最终状态的 Optional
     */
